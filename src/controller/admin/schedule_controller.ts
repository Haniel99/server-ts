...
const get_schedule_contoller = async (req: request, res: Response) => {
    try {
        ...
        return res.status(500).send( { status: false, response: "El usuario no tiene un horario creado"});
    } catch (error) {
        res.status(400).send({
            status: false,
            response: "Unexpected error occurred"
...
const get_schedule_semester_controller = async (req: request, res: Response) => {
    try {
        //get data of bady
        const { data } = req.body;
        //get schedule id
...
const create_class_block_controller = async (req: request, res: Response) => {
    try {
        ...
        const { data } = req.body;
        // Create new class block in the model
