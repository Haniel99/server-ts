...
const get_schedule_model = async (id: number) => {
  //sql for schedule
  let sql = `select  horario_id,plan,  semestre, fecha_creacion from horario where usuario_id = ${mysqlIntance.escape(
    id
  )} `;
  //query
...
const get_schedule_semester_model = async (id: number) => {
  //sql
  let sql: string = `select semestre_id, numero_semestre from semestre where horario_id = ${mysqlIntance.escape(
    id
  )}`;
  //query
  const res = await query(sql);
...
const create_semesters = async (id: number, data: Schedule) => {
  ...
  for (cont = 1; cont <= length; cont++) {
    //sql for semester
    let sql = `insert into semestre values(NULL,${mysqlIntance.escape(
      cont
    )}, ${mysqlIntance.escape(id)})`;
    await query(sql);
  }
};
...
const get_schedule_id = async (date: string, user_id: number) => {
  //sql
  let sql =
    `select horario_id from horario where fecha_creacion = ` +
...
const get_block_semester_model = async (semester_id: number) => {
  ...
  for (let i = 0; i < daysOfWeek.length; i++) {
    ...
    const sql = `
          ...
          WHERE a.semestre_id = ${mysqlIntance.escape(
            semester_id
          )} AND a.dia = ${mysqlIntance.escape(day)}
        `;
    const res = await query(sql);
...
const get_room_model = async (department_id: number) => {
  //Sql
  let sql = `select sala_id, nombre, número as numero from sala where departamento_id = ${mysqlIntance.escape(
    department_id
  )}`;
  //Query
  const res = await query(sql);
...
const putClassBlockModel = async (
  dataToVerify: IClassBlock,
  dataToChange: IClassBlockChange
) => {
  //sql
